...
import { canFetchUrl } from "~utils/canFetchUrl"
...
export const createBookmark = async (id: string, bookmark: BookmarkType) => {
  ...
  try {
    ...
    if (existing) {
      ...
      const isPublicPage = await canFetchUrl(bookmark.url)

      if (isPublicPage) {
        const summarizeRes = await summarizeAndExtractKeyInfo(bookmark.url)
        console.log("Tóm tắt:", summarizeRes)
        if (!summarizeRes.is_summarized) {
          summary = bookmark.title
          embedding = await generateEmbedding(summary)
        } else {
          summary = summarizeRes.summary
          key_info = summarizeRes.key_info as KeyInfoType
    ...
    if (error) {
      ...
      console.log("Bookmark đã được lưu thành công vào Supabase:", bookmarkData)
    }
  } catch (error) {
